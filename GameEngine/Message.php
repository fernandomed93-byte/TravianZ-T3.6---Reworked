...
class Message {
	...
	public $note;
	public $inbox, $inbox1, $sent, $sent1, $reading, $reply, $archived, $archived1, $readingNotice = [];
	private $totalMessage;
	public $noticearray = [];
	public $totalNotice = 0; // Para guardar o total
	public $reportsPerPage = 10; // Definir aqui ou buscar de config
	...
	public function noticeType($get) {
		...
		$sort_order = isset($get['o']) && $get['o'] == 1 ? 'ASC' : 'DESC';
		$types = [];
		$is_archived = 0; // Padrão: não arquivado

		if(isset($get['t'])) {
			$t = $get['t'];
			if($t == 1) $types = [8, 15, 16, 17]; // Reforço
			elseif($t == 2) $types = [10, 11, 12, 13]; // Comércio
			elseif($t == 3) { // Ataques
				$types = [1, 2, 3, 4, 5, 6, 7]; // Padrão: Todos os ataques

				// Filtro de Sentido (s): 1 = Enviados, 2 = Recebidos
				if(isset($get['s'])) {
					if($get['s'] == 1) {
						$types = array_intersect($types, [1, 2, 3]);
					} elseif($get['s'] == 2) {
						$types = array_intersect($types, [4, 5, 6, 7]);
					}
				}

				// Filtro de Resultado (r): 1 = Sem perdas, 2 = Parcial, 3 = Total
				if(isset($get['r'])) {
					if($get['r'] == 1) {
						// 1 (Env. sem perdas), 4 e 7 (Rec. sem perdas)
						$types = array_intersect($types, [1, 4, 7]);
					} elseif($get['r'] == 2) {
						// 2 (Env. parcial), 5 (Rec. parcial)
						$types = array_intersect($types, [2, 5]);
					} elseif($get['r'] == 3) {
						// 3 (Env. total), 6 (Rec. total)
						$types = array_intersect($types, [3, 6]);
					}
				}
				// Reindexar o array para o banco de dados
				$types = array_values($types);
			}
			elseif($t == 4) $types = [0, 18, 19, 20, 21]; // Diversos
			elseif($t == 5) { // Arquivados
				if(!$session->plus){
					header("Location: berichte.php");
					exit;
				}
				$is_archived = 1;
				$types = []; // Busca todos os tipos, mas apenas arquivados
			}
		} else {
			// Se 't' não está definido (aba "Todos"), busca tudo exceto arquivados
			$is_archived = 0;
			$types = []; // Busca todos os tipos não arquivados
		}

		$hasHero = (isset($_GET['h']) && $_GET['h'] == 1) ? true : false;

		$this->totalNotice = $database->getNoticeCount($session->uid, $types, $is_archived);
		$this->noticearray = $database->getNoticeLite($session->uid, $page, $types, $is_archived, $sort_order, $this->reportsPerPage, $hasHero);
		if(isset($get['id'])) {
	...
	private function filter_by_value_except($array, $index, $value) {
		// Return an empty array if the input isn't an array
		if (!is_array($array)) {
			return [];
		}

		// Use array_filter with a closure (anonymous function)
		return array_filter($array, function($item) use ($index, $value) {
			// The callback should return true for items to keep
			return isset($item[$index]) && $item[$index] != $value;
		});
	}
	
	private function filter_by_value($array, $index, $value_to_check) {
		// Garante que $value_to_check seja sempre um array
		if (!is_array($value_to_check)) {
			$value_to_check = [$value_to_check];
		}

		// Retorna um array vazio se a entrada não for um array
		if (!is_array($array)) {
			return [];
		}

		// Usa array_filter com uma closure
		return array_filter($array, function($item) use ($index, $value_to_check) {
			// Retorna true para itens a manter (se o índice existe E o valor está na lista)
			return isset($item[$index]) && in_array($item[$index], $value_to_check);
		});
	}
