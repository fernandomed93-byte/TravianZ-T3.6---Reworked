...
class Automation {
	...
	private $group_village_units_methods = [
        ...
        "trainingComplete", "starvation", "MasterBuilder", "sendSettlersComplete"
    ];
    ...
    private $group_accounts_alliances_methods = [
        ...
        "checkInvitedPlayes", "CheckBan", "medals", "loyaltyRegeneration"
    ];

    private $group_world_maintenance_methods = [
        "pruneResource", "pruneOResource", "updateStore", "clearDeleting",
        "delTradeRoute", "TradeRoute", "regenerateOasisTroops"
    ];
    ...
    public function __construct() {
        ...
        $automation_groups = [
            ...
            [
                ...
                'cooldown'  => 60,    // Apenas para testes
				//'cooldown'  => 1800,    // Segundos (30 minutos)
                'maxExecTime' => 900    // Segundos (15 minutos)
            ]
        ];
	...
	private function tryExecuteGroup($methodsArray, $lockFileName, $groupCooldown, $maxExecTime, $groupName = 'UnknownGroup') {
		global $database;
        ...
        if(isset($catapultTarget))
        {
            ...
            if ($newLevel == 0){       
                ...
                if ($isSecondRow) {
					...
					<img class=\"unit u".$catp_pic."\" src=\"gpack/travian_default/img/x.gif\" alt=\"Catapult\" title=\"Catapult\" /> ".$this->procResType($tbgid, $can_destroy)." <b>destroyed</b>.";
                    }
            ...
            elseif($newLevel == $tblevel){
                if($isSecondRow) {
                    if ($tbid > 0) {
                        $info_cat .= "<tbody class=\"goods\"><tr><th>Information</th><td colspan=\"11\">
					<img class=\"unit u".$catp_pic."\" src=\"gpack/travian_default/img/x.gif\" alt=\"Catapult\" title=\"Catapult\" /> ".$this->procResType($tbgid, $can_destroy)." was not damaged.</td></tr></tbody>";
                    }
                } else {
            ...
            { 
                ...
                if ($isSecondRow) {
                    $info_cat .= "<tbody class=\"goods\"><tr><th>Information</th><td colspan=\"11\">
					<img class=\"unit u".$catp_pic."\" src=\"gpack/travian_default/img/x.gif\" alt=\"Catapult\" title=\"Catapult\" /> ".$this->procResType($tbgid, $can_destroy).$info_cata;
            ...
            if(!isset($info_cat) || empty($info_cat) || $info_cat == ","){
                ...
                $info_cat .= "<tbody class=\"goods\"><tr><th>Information</th><td colspan=\"11\">
                          <img class=\"unit u".$catp_pic."\" src=\"gpack/travian_default/img/x.gif\" alt=\"Catapult\" title=\"Catapult\" /> There are no buildings left to destroy.</td></tr></tbody>";
            }       
        }
    }
	...
	private function processAttackArrival($data) {
		...
		$AttackerUserData = $database->getUserArray($database->getVillageField($data['from'],"owner"), 1);
		$Attacker['id'] = $AttackerUserData["id"];
		$AttackerID = $Attacker['id'];
		$owntribe = $AttackerUserData["tribe"];
		$ownally = $AttackerUserData["alliance"];
		$from = $database->getMInfo($data['from']);
		...
		if ($DefenderUserData == null || $DefenderUserData["id"] < 0 || $DefenderUserData["id"] == null){
			$returningTroops = [];
			for($i = 1; $i <= 11; $i++) $returningTroops['t'.$i] = $data['t'.$i];
			$troopsTime = $units->getWalkingTroopsTime($data['from'], $data['to'], $from['owner'], $owntribe, $returningTroops, 1, 't');
		...
		if ($isoasis == 0){
			...
			if($evasion == 1 && $maxevasion > 0 && $gold > 1 && !$cannotsend && $data['attack_type'] > 2){
				$evaded = true;
				$totaltroops = 0;
		...
		if(PEACE == 0 || $targettribe == 4 || $targettribe == 5 || $scout){
								...
								if(!isset($nochiefing)){
									...
									if (($toF['loyalty'] - $reducedLoyaltyTotal) > 0) {
										...
										$database->setVillageFields(
											$data['to'],
											['loyalty', 'owner', 'lastupdate2'],
											[0, $database->getVillageField($data['from'],"owner"), time()]
										);
										...
										$chefes_sobreviventes_da_batalha = $data['t9'] - $dead9 - (isset(${'traped9'}) ? ${'traped9'} : 0);

										if ($chefes_sobreviventes_da_batalha >= 1) {
											// Se pelo menos 1 chefe sobreviveu à batalha, 1 deles é "consumido" pela conquista.
											// Incrementamos $troopsdead9 para que addEnforce2 calcule 1 chefe a menos no reforço.
											$troopsdead9 = $dead9 + 1;
										}
			...
			if(($data['t11'] - $dead11 - $traped11) > 0){ //hero
				...
				if ($dead11 > 0) {
					...
					$database->KillMyHero($AttackerID); // LINHA ADICIONADA
				}
			}
			...
			if(!empty($scout)) {
				if ($data['spy'] == 1){
					$info_spy = "".$spy_pic.",<div class=\"res\"><img class=\"r1\" src=\"gpack/travian_default/img/x.gif\" alt=\"Lumber\" title=\"Lumber\" />".round($totwood)." |
					<img class=\"r2\" src=\"gpack/travian_default/img/x.gif\" alt=\"Clay\" title=\"Clay\" />".round($totclay)." |
					<img class=\"r3\" src=\"gpack/travian_default/img/x.gif\" alt=\"Iron\" title=\"Iron\" />".round($totiron)." |
					<img class=\"r4\" src=\"gpack/travian_default/img/x.gif\" alt=\"Crop\" title=\"Crop\" />".round($totcrop)."</div>
					<div class=\"carry\"><img class=\"car\" src=\"gpack/travian_default/img/x.gif\" alt=\"carry\" title=\"carry\" />Total Resources: ".round($totwood+$totclay+$totiron+$totcrop)."</div>
					";
				}else if($data['spy'] == 2){
					if ($isoasis == 0){
				...
				if(isset($village_destroyed) && $village_destroyed == 1 && $can_destroy==1){
					...
					if (strpos($info_cat, "The village has") === false) {
						$info_cat .= "<tbody class=\"goods\"><tr><th>Information</th><td colspan=\"11\">
								  <img class=\"unit u".$catp_pic."\" src=\"gpack/travian_default/img/x.gif\" alt=\"Catapult\" title=\"Catapult\" /> The village has been destroyed.</td></tr></tbody>";
					}
				}
		...
		unset(
			...
			,$can_destroy
			,$pop
			,$catapults1TargetRandom
			,$catapults2WillNotShoot
			,$catapults2TargetRandom
			,$totclay
			,$totiron
			,$totwood
			,$totcrop
            ...
            if ($vil['timetofinish'] <= time()) {
                ...
                if ($type == 10 || $type == 38) {
                    $database->query("
                        UPDATE ".TB_PREFIX."vdata
                            SET
                                `maxstore` = IF(`maxstore` - ".$buildarray[$level]['attri']." <= ".STORAGE_BASE.", ".STORAGE_BASE.", `maxstore` - ". ($buildarray[$level]['attri'] - $buildarray[($level - 1)]['attri']).")
                            WHERE
                                wref=".(int) $vil['vref']);
                }
                ...
                if ($type == 11 || $type == 39) {
                    $database->query("
                        UPDATE ".TB_PREFIX."vdata
                            SET
                                `maxcrop` = IF(`maxcrop` - ".$buildarray[$level]['attri']." <= ".STORAGE_BASE.", ".STORAGE_BASE.", `maxcrop` - ". ($buildarray[$level]['attri'] - $buildarray[($level - 1)]['attri']).")
                            WHERE
                                wref=".(int) $vil['vref']);
                }
	...
	/**
	 * Atualiza a capacidade de armazenamento de todas as aldeias de forma otimizada.
	 * Utiliza uma abordagem de atualização em lote para minimizar as consultas ao banco de dados.
	 */
	private function updateStore() {
		global $database, $bid10, $bid38, $bid11, $bid39;

		// FASE 1: Coleta de dados (igual ao original, mas usando streaming para segurança)
		$result = $database->query('SELECT * FROM `' . TB_PREFIX . 'fdata`');

		if (!$result) {
			error_log("updateStore: Falha ao executar a consulta inicial na tabela fdata.");
			return;
		}

		// FASE 2: Calcular tudo em memória
		$updates_to_make = [];
		while ($row = mysqli_fetch_assoc($result)) {
			...
			for ($i = 19; $i < 40; ++$i) {
				$building_type = $row['f' . $i . 't'];
				$building_level = $row['f' . $i];

				// Warehouse (Armazém)
				if ($building_type == 10) {
					$ress += ($bid10[$building_level]['attri'] ?? 0) * STORAGE_MULTIPLIER;
				}
				// Great Warehouse (Grande Armazém)
				else if ($building_type == 38) {
					$ress += ($bid38[$building_level]['attri'] ?? 0) * STORAGE_MULTIPLIER;
				}
				// Granary (Celeiro)
				else if ($building_type == 11) {
					$crop += ($bid11[$building_level]['attri'] ?? 0) * STORAGE_MULTIPLIER;
				}
				// Great Granary (Grande Celeiro)
				else if ($building_type == 39) {
					$crop += ($bid39[$building_level]['attri'] ?? 0) * STORAGE_MULTIPLIER;
				}
			}
			
			// Garante que a capacidade não seja menor que a base
			$final_ress = max($ress, STORAGE_BASE);
			$final_crop = max($crop, STORAGE_BASE);

			// Armazena os novos valores em um array, em vez de fazer a query agora
		...
		// FASE 3: Executar a atualização em lote (dividindo em pedaços de 500 para segurança)
		if (!empty($updates_to_make)) {
			$database->dblink->autocommit(FALSE); // Inicia a transação

			$chunks = array_chunk($updates_to_make, 500, true); // Divide o array em pedaços de 500
			...
			foreach ($chunks as $chunk) {
				...
				foreach ($chunk as $wref => $values) {
					$wref_ids[] = $wref;
					$whens_maxstore .= "WHEN " . (int)$wref . " THEN " . (int)$values['maxstore'] . " ";
					$whens_maxcrop .= "WHEN " . (int)$wref . " THEN " . (int)$values['maxcrop'] . " ";
				}

				$id_list = implode(', ', $wref_ids);

				$update_query = "UPDATE `" . TB_PREFIX . "vdata` SET " .
								"`maxstore` = CASE `wref` " . $whens_maxstore . "ELSE `maxstore` END, " .
								"`maxcrop` = CASE `wref` " . $whens_maxcrop . "ELSE `maxcrop` END " .
	...
	/**
	 * Processa a fome (starvation) para todas as aldeias do servidor de forma otimizada.
	 * Versão final e refatorada, utilizando processamento em streaming sincronizado
	 * para garantir alta performance e baixo uso de memória.
	 */
	private function starvation() {
		...
		// ===================================================================================
		// FASE 1: PREPARAÇÃO DAS "ESTEIRAS DE DADOS" (QUERIES ORDENADAS)
		// As consultas são executadas com ORDER BY para permitir o processamento sincronizado.
		// ===================================================================================

		$villageResult = $database->query("SELECT v.wref, v.owner, v.pop, v.starv AS old_starv, u.b4, u.tribe FROM ".TB_PREFIX."vdata v JOIN ".TB_PREFIX."users u ON v.owner = u.id WHERE v.owner > 5 ORDER BY v.wref ASC");
		$unitsResult = $database->query("SELECT * FROM ".TB_PREFIX."units ORDER BY vref ASC");
		$reinforcementsResult = $database->query("SELECT * FROM ".TB_PREFIX."enforcement ORDER BY vref ASC");
		$prisonersResult = $database->query("SELECT * FROM ".TB_PREFIX."prisoners ORDER BY wref ASC");
		$outgoingResult = $database->query("SELECT m.from AS home_village_wref, m.ref, m.moveid, a.* FROM ".TB_PREFIX."movement m JOIN ".TB_PREFIX."attacks a ON m.ref = a.id WHERE m.proc = 0 AND m.sort_type = 3 ORDER BY home_village_wref ASC");
		$returningResult = $database->query("SELECT m.to AS home_village_wref, m.ref, m.moveid, a.* FROM ".TB_PREFIX."movement m JOIN ".TB_PREFIX."attacks a ON m.ref = a.id WHERE m.proc = 0 AND m.sort_type = 4 ORDER BY home_village_wref ASC");
		$settlersResult = $database->query("SELECT m.from AS home_village_wref, m.ref as id, m.moveid FROM ".TB_PREFIX."movement m WHERE m.proc = 0 AND m.sort_type = 5 ORDER BY home_village_wref ASC");
		$all_active_artifacts = $database->query_return("SELECT * FROM ".TB_PREFIX."artefacts WHERE active = 1 AND type IN (4, 8) AND del = 0");
		$fdataResult = $database->query("SELECT * FROM ".TB_PREFIX."fdata ORDER BY vref ASC");
		$all_oases_raw = $database->query_return("SELECT conqured, type FROM ".TB_PREFIX."odata WHERE conqured > 0");


		// ===================================================================================
		// FASE 2: PROCESSAMENTO INTEGRADO (UMA ALDEIA POR VEZ)
		// ===================================================================================
		$cropProdByVillage = $this->calculate_all_crop_production($villageResult, $fdataResult, $all_oases_raw);

		// Mapas de referência rápidos que são pequenos o suficiente.
		$tribeOfOwner = [];
		$result_tribe_map = $database->query("SELECT id, tribe FROM ".TB_PREFIX."users");
		while($row = mysqli_fetch_assoc($result_tribe_map)) { $tribeOfOwner[$row['id']] = $row['tribe']; }
		mysqli_free_result($result_tribe_map);

		$artifacts_map = ['by_owner' => [], 'by_village' => []];
		foreach ($all_active_artifacts as $art) {
			$artifacts_map['by_owner'][$art['owner']][$art['type']] = $art;
			$artifacts_map['by_village'][$art['vref']][$art['type']] = $art;
		}

		// Pega a primeira linha de cada "esteira" de dados para iniciar.
		...
		$currentFdata = mysqli_fetch_assoc($fdataResult);
		
		

		// LOOP PRINCIPAL: Processa uma aldeia por completo antes de ir para a próxima.
		while ($village = mysqli_fetch_assoc($villageResult)) {
			$wref = $village['wref'];
			$upkeep = $village['pop'];
			
			$village_troops = ['units' => [], 'enforcements' => [], 'prisoners' => [], 'movements' => []];
			$village_buildings = [];

			// 2. Agora que está sincronizado, processa a linha se ela corresponder
			while ($currentFdata && $currentFdata['vref'] < $wref) { $currentFdata = mysqli_fetch_assoc($fdataResult); }
			if ($currentFdata && $currentFdata['vref'] == $wref) {
				$village_buildings = $currentFdata;
				$currentFdata = mysqli_fetch_assoc($fdataResult); // Avança para o próximo para a próxima iteração
			}
			
			// a) Processa tropas em casa
			...
			while ($currentUnit && $currentUnit['vref'] == $wref) {
				$upkeep += $this->calculate_upkeep_for_row($currentUnit, $village['tribe'], false, $village_buildings);
				$currentUnit['true_owner_id'] = $village['owner'];
				$village_troops['units'][] = $currentUnit;
				$currentUnit = mysqli_fetch_assoc($unitsResult);
			}

			// b) Processa reforços NA aldeia
			...
			while ($currentReinforcement && $currentReinforcement['vref'] == $wref) {
				...
				if (isset($tribeOfOwner[$owner_of_troops])) {
					$upkeep += $this->calculate_upkeep_for_row($currentReinforcement, $tribeOfOwner[$owner_of_troops], false, $village_buildings);
			...
			// c) Processa prisioneiros NA aldeia
			...
			while ($currentPrisoner && $currentPrisoner['wref'] == $wref) {
				...
				if (isset($tribeOfOwner[$owner_of_troops])) {
					$upkeep += $this->calculate_upkeep_for_row($currentPrisoner, $tribeOfOwner[$owner_of_troops], true, $village_buildings);
			...
			// d) Processa tropas em movimento DESTA aldeia
			...
			while ($currentOutgoing && $currentOutgoing['home_village_wref'] == $wref) {
				$upkeep += $this->calculate_upkeep_for_row($currentOutgoing, $village['tribe'], true, $village_buildings);
			...
			while ($currentReturning && $currentReturning['home_village_wref'] == $wref) {
				$upkeep += $this->calculate_upkeep_for_row($currentReturning, $village['tribe'], true, $village_buildings);
			...
			while ($currentSettler && $currentSettler['home_village_wref'] == $wref) {
				$settler_id = $village['tribe'] * 10;
				$upkeep += 3 * $GLOBALS['u'.$settler_id]['pop'];
				$currentSettler['true_owner_id'] = $village['owner'];
				$village_troops['movements'][] = $currentSettler;
				$currentSettler = mysqli_fetch_assoc($settlersResult);
			}

			// Geração da Lista Final e Execução da Fome (se necessário)
			$artifactMultiplier = $this->calculate_artifact_multiplier($village['owner'], $wref, $artifacts_map);
			$finalUpkeep = round($upkeep * $artifactMultiplier);
			$cropProd = $cropProdByVillage[$wref] ?? 0;

			if ($cropProd < $finalUpkeep) {
				$database->setVillageFields($wref, ['starv', 'starvupdate'], [$finalUpkeep, $time]);
				
				// FASE 3 COMEÇA AQUI, DENTRO DO LOOP PRINCIPAL
				$village['starv'] = $finalUpkeep; // Atualiza com o valor correto
				$village['starvupdate'] = $time; // Atualiza com o tempo correto
				$this->execute_starvation_for_village($village, $finalUpkeep, $village_troops, $tribeOfOwner, $cropProd);

			} else {
				if ($village['old_starv'] != 0) {
					$database->setVillageFields($wref, ['starv', 'starvupdate'], [0, 0]);
				}
			}
		...
		mysqli_free_result($fdataResult);
	}
	 ...
	 * @param int   $finalUpkeep        O consumo final de cereal já calculado.
	 ...
	 */
	private function execute_starvation_for_village($starv_data, $finalUpkeep, $village_troops, $tribeOfOwner, $cropProd) {
		global $database;
		...
		$timedif = $time - $starv_data['starvupdate'];
		$cropProd = $cropProd - $finalUpkeep;
		...
		if ($cropProd < 0) {
			...
			if ($oldcrop > 0) {
				...
				$database->modifyResource($wref, 0, 0, 0, $consumable_crop, 0);
			}
	...
	/**
	 * Calcula a produção de cereal para todas as aldeias do servidor de forma otimizada e em lote.
	 * Esta função substitui a necessidade de chamar getCropProdstarv repetidamente.
	 *
	 * @param array $villageDataMap     O mapa de aldeias [wref => dados] já carregado.
	 * @return array                    Um mapa [wref] => [produção_final_de_cereal]
	 */
	private function calculate_all_crop_production($villageResult, $fdataResult, $all_oases_raw) {
		global $database, $bid4, $bid8, $bid9;

		$cropProdByVillage = [];

		// Criação de mapas de referência rápidos a partir dos dados recebidos
		$buildingsByVillage = [];
		while ($f_row = mysqli_fetch_assoc($fdataResult)) {
			$buildingsByVillage[$f_row['vref']] = $f_row;
		}
		mysqli_data_seek($fdataResult, 0); // Rebobina se precisar usar de novo

		$oasesByVillage = [];
		foreach ($all_oases_raw as $oasis) {
			$oasesByVillage[$oasis['conqured']][] = $oasis['type'];
		}

		// Cálculo em memória para cada aldeia
		while ($village = mysqli_fetch_assoc($villageResult)) {
			$wref = $village['wref'];
			$basecrop = 0; $grainmill = 0; $bakery = 0;
			
			$buildarray = $buildingsByVillage[$wref] ?? [];
			if (empty($buildarray)) {
				$cropProdByVillage[$wref] = 0;
				continue;
			}

			// Soma a produção base dos campos de cereal (f1 a f18)
			for ($i = 1; $i <= 18; $i++) {
				if (isset($buildarray['f'.$i.'t']) && $buildarray['f'.$i.'t'] == 4) {
					$level = $buildarray['f'.$i];
					$basecrop += $bid4[$level]['prod'];
				}
			}
			// Encontra Moinho (gid 8) e Padaria (gid 9)
			for ($i = 19; $i <= 40; $i++) {
				if (isset($buildarray['f'.$i.'t'])) {
					if ($buildarray['f'.$i.'t'] == 8) $grainmill = $buildarray['f'.$i];
					if ($buildarray['f'.$i.'t'] == 9) $bakery = $buildarray['f'.$i];
				}
			}

			// Calcula o bônus percentual dos oásis
			$oasis_bonus_rate = 0;
			if (isset($oasesByVillage[$wref])) {
				foreach ($oasesByVillage[$wref] as $type) {
					if (in_array($type, [3, 6, 9, 10, 11])) $oasis_bonus_rate += 0.25;
					if ($type == 12) $oasis_bonus_rate += 0.50;
				}
			}

			// Aplica bônus de oásis à produção base
			$crop = $basecrop * (1 + $oasis_bonus_rate);
			
			// Aplica bônus de Moinho e Padaria, que também são sobre a produção base
			if ($grainmill > 0) $crop += $basecrop * ($bid8[$grainmill]['attri'] / 100);
			if ($bakery > 0) $crop += $basecrop * ($bid9[$bakery]['attri'] / 100);

			// Aplica bônus de +25% de cereal do usuário (Plus)
			if ($village['b4'] > time()) {
				$crop *= 1.25;
			}

			// CORREÇÃO FINAL: Aplica o multiplicador de velocidade do servidor
			$cropProdByVillage[$wref] = round($crop * SPEED);
		}
		 mysqli_data_seek($villageResult, 0);
		
		return $cropProdByVillage;
	}

	/**
	 * Calcula o consumo de cereal (upkeep) para uma única linha de dados de tropas.
	 * Esta versão considera a tribo do dono da tropa, o uso de 'pop' como consumo,
	 * e os efeitos de edifícios especiais como o Bebedouro de Cavalos.
	 *
	 * @param array $troop_row          A linha de dados contendo as contagens de tropas (de 'units', 'enforcement', etc.).
	 * @param int   $tribe_of_troops    A tribo do DONO das tropas (1=Rom, 2=Teut, 3=Gaul).
	 * @param bool  $is_t_column        (true) se as colunas de tropas forem t1-t11 (de 'movement' ou 'prisoners').
	 * @param array $village_buildings  A linha da tabela 'fdata' da aldeia que PAGA o consumo.
	 * @return int                      O consumo total de cereal calculado para aquela linha.
	 */
	private function calculate_upkeep_for_row($troop_row, $tribe_of_troops, $is_t_column, $village_buildings) {
		$upkeep = 0;

		// Verifica o nível do Bebedouro de Cavalos (gid 41) na aldeia que paga o consumo.
		$horsedrinking_lvl = 0;
		if (!empty($village_buildings)) {
			for ($i = 19; $i <= 40; $i++) {
				if (isset($village_buildings['f'.$i.'t']) && $village_buildings['f'.$i.'t'] == 41) {
					$horsedrinking_lvl = (int)$village_buildings['f'.$i];
					break;
				}
			}
		}

		$start_index = ($tribe_of_troops - 1) * 10 + 1;
		
		// Lógica para colunas u1-u50, hero (tabelas units, enforcement)
		if (!$is_t_column) {
			for ($i = $start_index; $i < $start_index + 10; $i++) {
				$unit_column = 'u' . $i;
				if (isset($troop_row[$unit_column]) && $troop_row[$unit_column] > 0) {
					$unit_data = $GLOBALS[$unit_column];
					
					// CORREÇÃO: Usando 'pop' para o consumo de cereal, como no código original.
					$base_consumption = $unit_data['pop'];
					
					// Aplica o efeito do Bebedouro para as tropas de cavalaria corretas (Romanos)
					if ($horsedrinking_lvl > 0 && $tribe_of_troops == 1) {
						if (($i == 4 && $horsedrinking_lvl >= 10) || ($i == 5 && $horsedrinking_lvl >= 15) || ($i == 6 && $horsedrinking_lvl == 20)) {
							$base_consumption -= 1;
						}
					}
					$upkeep += $troop_row[$unit_column] * $base_consumption;
				}
			}
			if (isset($troop_row['hero']) && $troop_row['hero'] > 0) {
				$upkeep += $troop_row['hero'] * 6; // Heróis sempre consomem 6
			}
		} 
		// Lógica para colunas t1-t11 (tabelas movement, prisoners)
		else { 
			for ($i = 1; $i <= 10; $i++) {
				$troop_column = 't' . $i;
				if (isset($troop_row[$troop_column]) && $troop_row[$troop_column] > 0) {
					$unit_id = $start_index + $i - 1;
					$unit_data = $GLOBALS['u' . $unit_id];
					
					// CORREÇÃO: Usa 'pop' para o consumo de cereal.
					$base_consumption = $unit_data['pop'];

					// Aplica o efeito do Bebedouro
					if ($horsedrinking_lvl > 0 && $tribe_of_troops == 1) {
						if (($unit_id == 4 && $horsedrinking_lvl >= 10) || ($unit_id == 5 && $horsedrinking_lvl >= 15) || ($unit_id == 6 && $horsedrinking_lvl == 20)) {
							$base_consumption -= 1;
						}
					}
					$upkeep += $troop_row[$troop_column] * $base_consumption;
				}
			}
			if (isset($troop_row['t11']) && $troop_row['t11'] > 0) {
				$upkeep += $troop_row['t11'] * 6;
			}
		}
		return $upkeep;
	}
	...
	/**
	 * Calcula o multiplicador de consumo de cereal com base nos artefatos de um jogador/aldeia.
	 * Opera sobre um array pré-carregado para máxima performance.
	 *
	 * @param int   $uid                    O ID do dono da aldeia.
	 * @param int   $vid                    O wref da aldeia.
	 * @param array $artifacts_map          Array pré-processado com todos os artefatos relevantes (tipo 4 e 8).
	 * @return float                        O multiplicador de consumo (ex: 1.0 para normal, 0.75 para artefato pequeno).
	 */
	private function calculate_artifact_multiplier($uid, $vid, $artifacts_map) {
		// Multiplicadores padrão: 1 = Pequeno, 2 = Grande, 3 = Único
		$multipliers = [
			// Efeitos do "Diet Control" (type 4)
			'4' => [
				1 => 0.50, // Artefato Pequeno (Vila): 50% de economia
				2 => 0.75, // Artefato Grande (Conta): 25% de economia
				3 => 0.50, // Artefato Único (Conta): 50% de economia
			],
		];

		$final_multiplier = 1.0;
		$has_normal_artifact = false;

		// --- Procura por artefatos de Dieta (Tipo 4) ---
		// 1. Efeito de Aldeia (prioridade mais alta para artefatos normais)
		if (isset($artifacts_map['by_village'][$vid][4])) {
			$artifact = $artifacts_map['by_village'][$vid][4];
			$final_multiplier = $multipliers[4][$artifact['size']];
			$has_normal_artifact = true;
		}
		// 2. Efeito de Conta (se nenhum de aldeia foi encontrado)
		if (!$has_normal_artifact && isset($artifacts_map['by_owner'][$uid][4])) {
			$artifact = $artifacts_map['by_owner'][$uid][4];
			if ($artifact['size'] > 1) { // Artefatos de conta só funcionam se forem grandes ou únicos
				$final_multiplier = $multipliers[4][$artifact['size']];
				$has_normal_artifact = true;
			}
		}

		// --- Procura por Artefato do Louco (Tipo 8) que pode sobrescrever o efeito ---
		$fool_effect = null;
		// 1. Efeito de Aldeia
		if (isset($artifacts_map['by_village'][$vid][8])) {
			$fool_artifact = $artifacts_map['by_village'][$vid][8];
			if ($fool_artifact['kind'] == 4) { // O Louco está imitando o de Dieta
				$fool_effect = ($fool_artifact['bad_effect'] == 1) ? (1 / $fool_artifact['effect2']) : $fool_artifact['effect2'];
			}
		}
		// 2. Efeito de Conta (se nenhum de aldeia foi encontrado)
		if ($fool_effect === null && isset($artifacts_map['by_owner'][$uid][8])) {
			$fool_artifact = $artifacts_map['by_owner'][$uid][8];
			if ($fool_artifact['size'] > 1 && $fool_artifact['kind'] == 4) {
				$fool_effect = ($fool_artifact['bad_effect'] == 1) ? (1 / $fool_artifact['effect2']) : $fool_artifact['effect2'];
			}
		}

		// O efeito do Artefato do Louco sempre tem prioridade sobre os outros.
		if ($fool_effect !== null) {
			// Para artefatos de redução, o efeito é inverso
			$final_multiplier = 1 / $fool_effect;
		}
