  ...
  // no need to cache this method
	...
	*****************************************/
	...
	function getFieldDistance($wid) {
        ...
        $array = $this->getProfileVillages($wid, 3); // Lista de vilas de outros usuarios
        $prevdist = 0;
...
***************************/
	...
	***************************/
    ...
    function getUnfinishedMovements($type){ // 0 Ataques 1 Reforços 2 Retornos
		$time = time();
		switch($type) {
			case 0: //ATAQUES
					$q = "
	...
	function getUnatVillages($type, $village, $mode, $use_cache = true){
		$array_passed = is_array($village);
		if (!$array_passed) {
            $village = [(int) $village];
        } else {
		...
		$vilInfo = $this->getMInfo(implode(', ', $village));
		$vilInfox = $vilInfo['x'];
		$vilInfoy = $vilInfo['y'];
		
		$q = "SELECT vdat.wref, vdat.name, wdat.x, wdat.y, users.username, (vdat.wood + vdat.clay + vdat.iron + vdat.crop) as recursos, SQRT((".$vilInfox." - wdat.x)*(".$vilInfox." - wdat.x) + (".$vilInfoy." - wdat.y)*(".$vilInfoy." - wdat.y)) as dist FROM " . TB_PREFIX . "vdata as vdat 
				INNER JOIN " . TB_PREFIX . "wdata as wdat ON vdat.wref=wdat.id
				INNER JOIN " . TB_PREFIX . "users as users ON vdat.owner=users.id
				WHERE NOT EXISTS (SELECT * FROM " . TB_PREFIX . "movement, " . TB_PREFIX . "attacks where " . TB_PREFIX . "movement.from IN(".implode(', ', $village).") and " . TB_PREFIX . "movement.ref = " . TB_PREFIX . "attacks.id and " . TB_PREFIX . "movement.proc = 0 and " . TB_PREFIX . "movement.sort_type = 3 and vdat.wref = " . TB_PREFIX . "movement.to)
				AND NOT EXISTS   (SELECT * FROM " . TB_PREFIX . "movement, " . TB_PREFIX . "attacks where " . TB_PREFIX . "movement.to IN(".implode(', ', $village).") and " . TB_PREFIX . "movement.ref = " . TB_PREFIX . "attacks.id and " . TB_PREFIX . "movement.proc = 0 and " . TB_PREFIX . "movement.sort_type = 4 and vdat.wref = " . TB_PREFIX . "movement.from)
				AND NOT EXISTS (SELECT * FROM " . TB_PREFIX . "vdata WHERE " . TB_PREFIX . "vdata.wref IN(".implode(', ', $village).") and " . TB_PREFIX . "vdata.owner = vdat.owner )
				AND vdat.pop > 150 ORDER BY dist ASC LIMIT 25";
			$result = $this->mysqli_fetch_all(mysqli_query($this->dblink,$q));
	...
	***************************/
    ...
    public function canClaimArtifact($from, $vref, $size, $type) {
        ...
        if ($artifact['totals'] < 6 || $uid == $vuid) {
            ...
            if(($artifact['great'] > 5 || $artifact['unique'] > 5) && $size > 1 && $uid != $vuid) {
