...
class Market
{
    ...
    private function sendResource($post)
    {
        ...
        if($database->getvacmodexy($village->wid)) $form->addError("error", USER_ON_VACATION);
    ...
    private function tradeResource($post)
    {
        ...
        if($wwvillage['f99t'] != 40){
            if($session->userinfo['gold'] >= 3){
                ...
                $totalAvailable = (int)$post['m1'][0] + (int)$post['m1'][1] + (int)$post['m1'][2] + (int)$post['m1'][3];
                $totalToTrade = (int)$post['m2'][0] + (int)$post['m2'][1] + (int)$post['m2'][2] + (int)$post['m2'][3];
                $tolerancia = floor($totalAvailable * 0.01);
                ...
                if ($totalToTrade <= ($totalAvailable + $tolerancia)) {
