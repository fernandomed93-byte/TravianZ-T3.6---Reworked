...
class Account {
	...
	private function Signup() {
		...
		if(!isset($_POST['vid']) || !in_array($_POST['vid'], [1, 2, 3, 6, 7, 8, 9])) {
		...
		if($form->returnErrors() > 0) {
			...
			$uid = $database->activate($_POST['name'],password_hash($_POST['pw'], PASSWORD_BCRYPT,['cost' => 12]),$_POST['email'],$_POST['vid'],$_POST['kid'],$act,$act2);
				...
				if($uid) {
					setcookie("COOKUSR_T3" , $_POST['name'], time() + COOKIE_EXPIRE,COOKIE_PATH);
					setcookie("COOKEMAIL" , $_POST['email'], time() + COOKIE_EXPIRE,COOKIE_PATH);
					$database->updateUserField(
						$uid,"invited", $_POST['invited'],1
                    );
	...
	private function Login() {
		...
		if($form->returnErrors() > 0){
			...
			setcookie("COOKUSR_T3", $_POST['user'], time() + COOKIE_EXPIRE, COOKIE_PATH);
