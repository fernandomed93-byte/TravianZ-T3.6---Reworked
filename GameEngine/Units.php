...
class Units {
    ...
    public function checkErrors(&$post){
        ...
        if(isset($post['x']) && isset($post['y']) && $post['x'] != "" && $post['y'] != "") {
            ...
            $id = $generator->getBaseIDMAP($coor['x'], $coor['y']);
        ...
        if (!empty($coor)) {
            ...
            for($i = 1; $i < 12; $i++){
                if(isset($post['t'.$i])){
                    ...
                    if(intval($post['t'.$i]) > $troophave) return " " . $post['t'.$i] . " - You can't send more units than you have";
    ...
    private function processReturnTroops($enforce) {
        ...
        $time = $database->getArtifactsValueInfluence($to['owner'], $enforce['from'], 2, $troopsTime);
    ...
    private function sendTroops($post) {
        ...
        if($data['type'] != 1 && isset($post['spy']) && $post['spy'] != 0) $post['spy'] = 0;
