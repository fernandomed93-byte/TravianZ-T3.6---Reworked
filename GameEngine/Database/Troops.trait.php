...
trait DBTroops
{
    ...
    {
        $q = "SELECT * FROM " . TB_PREFIX . "training where vref IS NOT NULL AND timestamp2 <= " . time();
        $result = mysqli_query($this->dblink, $q);
        return $this->mysqli_fetch_all($result);
    }
    ...
    public function getWoundedData($vref)
    {
        ...
        $qRow = mysqli_fetch_assoc(mysqli_query(
            $this->dblink,
            "SELECT
    ...
    function getTrainingLock($wid) {
        $wid = (int) $wid;
        $result = mysqli_query($this->dblink, "SELECT GET_LOCK('train_village_$wid', 1) AS locked");
        $row = mysqli_fetch_assoc($result);
        return $row['locked'] == 1;
    }
    ...
    function getEnforceLock($id) {
        $id = (int) $id;
        $result = mysqli_query($this->dblink, "SELECT GET_LOCK('enforce_$id', 1) AS locked");
        $row = mysqli_fetch_assoc($result);
        return $row['locked'] == 1;
    }
