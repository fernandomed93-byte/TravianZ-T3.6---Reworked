...
class Building {
    ...
    public function procBuild($get) {
        ...
        if(isset($get['a']) && $get['c'] == $session->checker && !isset($get['id'])) {
            ...
            if($get['id'] > 18 && ($get['id'] < 41 || $get['id'] == 99)){
				if ($this->getLevelField($get['id']) < 1) {
            	$session->changeChecker();
	...
	public static function procResType($ref) {
		switch($ref) {
			...
			case 42: return STONEWALL;
			case 43: return MAKESHIFTWALL;
			case 44: return COMMANDCENTER;
			case 45: return WATERWORKS;
			case 46: return HOSPITAL;
			case 47: return DEFENSIVEWALL;
			case 48: return BIGHOSPITAL;
			case 49: return GREATWORKSHOP;
			case 50: return BARRICADE;
	...
	private function downgradeBuilding($id) {
		...
		if($this->allocated < $this->maxConcurrent) {
			...
			global $$name;
			$dataarray = $$name;
			$time = time() + round($dataarray[$village->resarray['f'.$id]]['time'] / 4);
			$loop = 0;
			...
			if($loop == 1) {
				if($session->tribe == 1 || ALLOW_ALL_TRIBE) {
					if($id >= 19) {
						foreach($this->buildArray as $build) {
							if($build['field'] >= 19) {
								$time = $build['timestamp'] + round($dataarray[$village->resarray['f'.$id]]['time'] / 4);
							}
						}
					}
				}
				else {
					$time = $this->buildArray[0]['timestamp'] + round($dataarray[$village->resarray['f'.$id]]['time'] / 4);
				}
			}
	...
	private function constructBuilding($id, $tid) {
		...
		if($this->allocated < $this->maxConcurrent) {              
		    if($tid == 16) $id = 39;
		    elseif(in_array($tid, [31,32,33,42,43,47,50])) $id = 40;
    ...
    public function finishAll($redirect_url = '') {
        global $database, $session, $logging, $village, $bid18, $bid10, $bid11, $bid38, $bid39, $technology, $_SESSION;
		
        $countPlus2Gold  = false; // will be true if we should decrease player's gold by 2 for the immediate completion action
        $countMasterGold = false; // will be true if we should decrease player's gold by 1 for master builder queue
        $buildcount = ($this->buildArray ? count($this->buildArray) : 0); // number of jobs to finish
        $jobFinishSuccess = false; // will be true if the job was successfully finished
        $deletIDs = []; // IDs of successful jobs to delete
		$firstJobOriginalTimestamp = 0;
		$liveCapacities = []; // Array para armazenar a capacidade do armazem ou celeiro antes do update
		$enought_res = 0;
        ...
        foreach ($this->buildArray as $jobs) {
            if ($jobs['wid'] == $village->wid) {
                ...
                if ($wwvillage['f99t'] != 40 || $bypass) { // Não é Vila de WW
                    ...
                    if ($jobs['type'] != 25 && $jobs['type'] != 26 && $jobs['type'] != 44 && $jobs['type'] != 40) { //Também não é Residencia, Palácio ou WW
                        ...
                        } else {// non-master builder build, we should count +2 gold for it
                            
                            $countPlus2Gold = true;
                            $jobFinishSuccess = true;
							if ($firstJobOriginalTimestamp == 0) {
								$firstJobOriginalTimestamp = $jobs['timestamp'];
							}
                        }
                        ...
                        if (!isset($exclude_master) && $database->query($q) && ($enought_res == 1 || $jobs['master'] == 0)) {
							...
							if (in_array($jobs['type'], [10, 11, 38, 39])) {
								$fieldDbName = (in_array($jobs['type'], [10, 38]) ? 'maxstore' : 'maxcrop');
								
								// Usamos o rastreador para obter o valor mais recente
								if (!isset($liveCapacities[$jobs['wid']][$fieldDbName])) {
									// Se for a primeira vez, lemos do banco de dados
									$liveCapacities[$jobs['wid']][$fieldDbName] = $database->getVillageField($jobs['wid'], $fieldDbName);
								}
								$currentMax = $liveCapacities[$jobs['wid']][$fieldDbName];
								if ($currentMax == 800){$currentMax = 0;}
								
								// Mesma lógica de cálculo de 'buildComplete'
								if ($jobs['level'] > 1) { // Usamos > 1 para maior clareza
									// Usamos a variável global correta para o tipo de edifício
									$currentMax -= ${'bid'.$jobs['type']}[$jobs['level'] - 1]['attri'] * STORAGE_MULTIPLIER;
								}
								$newMax = $currentMax + ${'bid'.$jobs['type']}[$jobs['level']]['attri'] * STORAGE_MULTIPLIER;
								
								// Atualiza o valor no rastreador "ao vivo"
								$liveCapacities[$jobs['wid']][$fieldDbName] = $newMax;
								
								// Salva o novo valor imediatamente no banco de dados
								$database->setVillageField($jobs['wid'], $fieldDbName, $newMax);
							}
            ...
            $jobFinishSuccess = false; // reset the flag for the next job
        }
        ...
        $demolition = $this->executeDemolition($village->wid);
        ...
        if (count($stillbuildingarray) == 1 && $firstJobOriginalTimestamp > 0) {
			$nextJob = $stillbuildingarray[0];
            if($nextJob['loopcon'] == 1) {
				$time_saved = $firstJobOriginalTimestamp - time();
				$time_saved = max(0, $time_saved);
				$newTimestamp = $nextJob['timestamp'] - $time_saved;
				
                $q = "UPDATE ".TB_PREFIX."bdata SET loopcon=0, timestamp=" . $newTimestamp . " WHERE id=".(int) $nextJob['id'];
                //$q = "UPDATE ".TB_PREFIX."bdata SET loopcon=0 WHERE id=".(int) $stillbuildingarray[0]['id'];
                $database->query($q);
            }
        }
        
		self::recountPop($database, $village->wid);
	    
        header("Location: ".($redirect_url ? $redirect_url : $session->referrer));
        exit;
    }
	
	function executeDemolition($wid) {
    	global $database;
        $varray = $database->getDemolition($wid);
		$countDems = 0;
	...
	function recountPop($database, $vid, $use_cache = true){
	...
	public function getLevelField($type) {
		global $village;
		return $village->resarray['f'.$type];
	}
	...
	public function calculateAvaliable($id, $tid, $plus = 1) {
		...
		public function calculateAvaliableTotal($id, $tid, $plus = 1) {
		global $village, $generator;
		...
		$totalReq = round($rwood + $rclay + $rcrop + $riron);
		
		$totalProd = ($village->getProd("wood") + $village->getProd("clay") + $village->getProd("crop") + $village->getProd("iron")) / 3600 ;
		$totalTime = $totalReq / $totalProd;
		$totalTime += time();

		return $generator->procMtime($totalTime);
	}
};
