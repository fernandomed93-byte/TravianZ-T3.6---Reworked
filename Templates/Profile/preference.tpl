...
if($_POST) {
    ...
    foreach($links as $link) {
        ...
        if(trim($link['nr']) != '' AND trim($link['linkname']) != '' AND trim($link['linkziel']) != '' AND (!isset($link['id']) OR trim($link['id']) == '')) {
            // Add new link
...
<form action="spieler.php?s=2" method="POST">
  ...
  <table cellpadding="1" cellspacing="1" id="links">
    ...
    <tbody>
	  <?php $last_pos = 0; $i = 0; foreach($links as $link): ?>
      <tr>
	  <td>
	  <a href="spieler.php?del=<?php echo $link['id']; ?>&s=2"><img class="del" src="gpack/travian_default/img/x.gif" alt="delete" title="delete"></a>
	  </td>
	...
	<td class="txt"><input <?php if(!$session->plus){echo"disabled";} ?> class="text" type="text" name="linkziel<?php print $i; ?>" value="" maxlength="255"></td>
      ...
      <tr>
       <td colspan="4"><input type="image" value="" name="s1" id="btn_ok" class="dynamic_img" src="gpack/travian_default/img/x.gif" alt="OK" /></td>
      </tr>
    </tbody>
  </table>
</form>
...
</table><p class="btn"><input type="image" value="" name="s1" id="btn_ok" class="dynamic_img" src="gpack/travian_default/img/x.gif" alt="OK" /></p>
