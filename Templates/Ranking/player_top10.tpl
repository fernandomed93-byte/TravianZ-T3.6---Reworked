    ...
    $result = mysqli_query($database->dblink,"SELECT * FROM ".TB_PREFIX."users WHERE access<".(INCLUDE_ADMIN?"10":"8")." AND id > 5 AND tribe > 0 AND (tribe<=3 OR tribe > 5) ORDER BY ap DESC, id DESC Limit 10");
...
<div id="submenu">
<a title="Top 10" href="statistiken.php?id=7"><img class="active btn_top10" src="gpack/travian_default/img/x.gif" alt="Top 10"></a>
<a title="Defenders" href="statistiken.php?id=32"><img class="btn_def" src="gpack/travian_default/img/x.gif" alt="Defenders"></a>
<a title="Attackers" href="statistiken.php?id=31"><img class="btn_off" src="gpack/travian_default/img/x.gif" alt="Attackers"></a>
</div>
</div>
<table cellpadding="1" cellspacing="1">
	<thead>
		<tr>
			<th>Top 10 of the Week</th>
		</tr>
	</thead>
</table>
<table cellpadding="1" cellspacing="1" id="top10_offs" class="top10 row_table_data">
	<thead>
		<tr>
			<th onclick="return Popup(3,5)"><img src="gpack/travian_default/img/x.gif" class="help" alt="Instructions" title="Instructions">
			</th>
...
<?php
    while($row = mysqli_fetch_array($result))
      ...
      $usernameExibicao = mb_strimwidth($row['username'], 0, 16, "...");
	  echo "<td class=\"pla\"><a href='spieler.php?uid=".$row['id']."' title='".$row['username']."'>".$usernameExibicao."</a></td>";
...
<?php
    ...
    $result = mysqli_query($database->dblink,"SELECT * FROM ".TB_PREFIX."users WHERE access<".(INCLUDE_ADMIN?"10":"8")." AND id > 5 AND tribe > 0 AND (tribe<=3 OR tribe > 5) ORDER BY dp DESC, id DESC Limit 10");
...
<?php
    ...
    $result = mysqli_query($database->dblink,"SELECT * FROM ".TB_PREFIX."users WHERE access<".(INCLUDE_ADMIN?"10":"8")." AND id > 5 AND tribe > 0 AND (tribe<=3 OR tribe > 5) ORDER BY clp DESC, id DESC Limit 10");
...
<?php
    ...
    $result = mysqli_query($database->dblink,"SELECT * FROM ".TB_PREFIX."users WHERE access<".(INCLUDE_ADMIN?"10":"8")." AND id > 5 AND tribe > 0 AND (tribe<=3 OR tribe > 5) ORDER BY RR DESC, id DESC Limit 10");
...
<?php
    while($row = mysqli_fetch_array($result2))
      ...
      if($row['id']==$session->uid) {
		echo "<tr class=\"none\">"; } 
		else { echo "<tr class=\"own hl\">"; }

      echo "<td class=\"ra fc\">".$place3."&nbsp;</td>";
      if($row['id']==$session->uid) {
		echo "<td class=\"pla\">".$row['username']."</td>"; } 
		else { echo "<td class=\"pla\"><a href='spieler.php?uid=".$row['id']."'>".$row['username']."</a></td>"; }
