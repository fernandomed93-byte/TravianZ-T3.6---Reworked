...
for($i=1;$i<=18;$i++) {
	if($arrayVillage['f'.$i.'t'] != 0) {
		...
		echo "<img src=\"gpack/travian_default/img/x.gif\" class=\"reslevel rf$i level".$arrayVillage['f'.$i].(isset($activeFields[$i]) ? '_active' : '')."\" alt=\"$text ".$arrayVillage['f'.$i].(isset($activeFields[$i]) ? ' (upgrade in progress)' : '')."\" />";
	}
}
?>
	<img id="resfeld" usemap="#rx" src="gpack/travian_default/img/x.gif" alt="" />
</div>