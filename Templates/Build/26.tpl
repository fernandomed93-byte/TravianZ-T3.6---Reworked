...
if($_POST and $_GET['action'] == 'change_capital' && !$village->capital){
	...
	if(password_verify($pass, $data['password'])){
		...
		$querynewcapital = mysqli_query($database->dblink, 'SELECT * FROM `'.TB_PREFIX.'fdata` WHERE `vref` = '.(int)$village->wid);
		$data3 = mysqli_fetch_assoc($querynewcapital);
		...
		if($data2['vref'] != $village->wid){
			...
			for($i = 19; $i <= 40; ++$i){ //Stonemason's Lodge can only be built in the capital
				if($data2['f'.$i.'t'] == 34){
			...
			for($i = 19; $i <= 40; ++$i){
				if($data3['f'.$i.'t'] == 29 || $data3['f'.$i.'t'] == 30 || $data3['f'.$i.'t'] == 42){
...
<p class="build_desc">
        <a href="#" onClick="return Popup(26,4, 'gid');"
                ...
                class="building g26"
                src="gpack/travian_default/img/x.gif" alt="Palace"
...
<input type="image" id="btn_ok" class="dynamic_img" value="ok" name="s1" src="gpack/travian_default/img/x.gif" alt="train" />
</form>
</p>';
  }
}
