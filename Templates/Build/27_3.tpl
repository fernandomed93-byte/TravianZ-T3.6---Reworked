<body>
    <div id="build" class="gid27">
        <a href="#" onclick="return Popup(27,4);" class="build_logo"><img class="building g27" src="gpack/travian_default/img/x.gif" alt="Treasury" title="<?php echo TREASURY; ?>"></a>
        ...
        <table id="show_artefacts" cellpadding="1" cellspacing="1">
            ...
            if(count($artifactsArray) == 0) {
                ...
                foreach($artifactsArray as $artifact){
                    ...
                    echo '<tr>
                              <td class="icon"><img class="artefact_icon_'.$artifact['type'].'" src="gpack/travian_default/img/x.gif" alt="" title=""></td>
                              <td class="nam">';
							  $percentEffect = $artifact['effect'];
								if ($artifact['type'] == 6) $percentEffect = "";
                                  echo '<a href="build.php?id='.$id.'&show='.$artifact['id'].'">'.$artifact['name'] . '</a> <span class="bon">'.$percentEffect.'</span><div class="info">'.TREASURY.' <b>20</b>, '.EFFECT.' <b>'.ACCOUNT.'</b></div>
                              </td>
