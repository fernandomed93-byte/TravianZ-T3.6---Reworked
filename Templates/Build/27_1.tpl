...
else 
{
    foreach($ownArtifacts as $ownArtifact){
		...
		$percentEffect = $ownArtifact['effect'];
		if ($ownArtifact['type'] == 6) $percentEffect = "";
        echo '<tr><td class="icon"><img class="artefact_icon_' . $ownArtifact['type'] . '" src="gpack/travian_default/img/x.gif"></td>';
        echo '<td class="nam">
                <a href="build.php?id='.$id . '&show='.$ownArtifact['id'].'">' . $ownArtifact['name'] . '</a> <span class="bon">' . $percentEffect . '</span>
                <div class="info">
...
else
{
    ...
    foreach($rows as $distance => $row) {
        echo '<tr>
                <td class="icon"><img class="artefact_icon_'.$row['type'].'" src="gpack/travian_default/img/x.gif" alt="" title=""></td>
                <td class="nam">';
				$percentEffect = $row['effect'];
				if ($row['type'] == 6) $percentEffect = "";
                echo '<a href="build.php?id='.$id.'&show='.$row['id'].'">'.$row['name'].'</a> <span class="bon">'.$percentEffect.'</span>
              <div class="info">';
