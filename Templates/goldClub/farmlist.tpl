...
?>
                        <div class="listTitleText">
							<a href="build.php?gid=16&t=99&action=deleteList&lid=<?php echo $lid; ?>"><img class="del" src="gpack/travian_default/img/x.gif" alt="delete" title="delete"></a>
...
<td class="checkbox">
            ...
            if (($attacksCount = count($attacks)) > 0) {
                ...
                if($attacksCount > 0) echo '<img class="att2" src="gpack/travian_default/img/x.gif" title="'.OWN_ATTACKING_TROOPS.'" />';
            }
        	?>
...
<?php
    ...
    for($i = $start; $i <= $end; $i++){
    	if(${'t'.($i - $start + 1)} > 0){
    		echo '<div class="troopIcon"><img class="unit u'.$i.'" title="'.$technology->getUnitName($i).'" src="gpack/travian_default/img/x.gif"><span class="troopIconAmount">'.${'t'.($i - $start + 1)}.'</span></div>';
    	}
    }
?>
...
while($row2 = mysqli_fetch_array($getnotice)){
    ...
    $type2 = $row2['ntype'];
    echo "<img src=\"gpack/travian_default/img/x.gif\" class=\"iReport iReport".$row2['ntype']."\" title=\"".$noticeClass[$type2]."\"> ";
    
    $allres = $dataarray[23] + $dataarray[24] + $dataarray[25] + $dataarray[26];
    $carry = $dataarray[27];

    echo "<img title=\"$allres/$carry\" src=\"gpack/travian_default/img/x.gif\" class=\"carry\"/>";
