...
if(isset($_GET['z'])) {
	...
	$x = $_POST['xp'];
    $y = $_POST['yp'];
    $bigmid = $generator->getBaseIDMAP($x,$y);
}else{
    $y = $village->coor['y'];
	$x = $village->coor['x'];
    $bigmid = $village->wid;
...
for($i=0; $i<=6; $i++){
	if($xcount != 7){
		$maparray .= '\''.$generator->getBaseIDMAP($xarray[$xcount],$yarray[$i]).'\',';
		$maparray2 .= $generator->getBaseIDMAP($xarray[$xcount],$yarray[$i]).',';
		if($i==6){
			$i = -1;
			$xcount +=1;
		}
...
//echo $x . " | " . $y . " | ".$generator->getBaseIDMAP($xm3,$yp3)."| " . $maparray;
...
<div id="content"  class="map">
	...
	<div id="map">
		...
		<map id="map_overlay" name="map_overlay">
			<?php echo $map_gen;?>
			<area id="ma_n1" href="karte.php?z=<?php echo $generator->getBaseIDMAP($x,$yp1);?>" coords="422,67,25" shape="circle" title="<?php echo NORTH;?>"/>
			<area id="ma_n2" href="karte.php?z=<?php echo $generator->getBaseIDMAP($xp1,$y);?>" coords="427,254,25" shape="circle" title="<?php echo EAST;?>"/>
			<area id="ma_n3" href="karte.php?z=<?php echo $generator->getBaseIDMAP($x,$ym1);?>" coords="119,255,25" shape="circle" title="<?php echo SOUTH;?>"/>
			<area id="ma_n4" href="karte.php?z=<?php echo $generator->getBaseIDMAP($xm1,$y);?>" coords="114,63,25" shape="circle" title="<?php echo WEST;?>"/>
		</map>
		<img id="map_links" src="gpack/travian_default/img/x.gif" usemap="#map_overlay" />
		<script type="text/javascript">
			m_c.az = {<?php echo '"n1":'.$generator->getBaseIDMAP($x,$yp1).',"n1p7":'.$generator->getBaseIDMAP($x,$yp7).',"n2":'.$generator->getBaseIDMAP($xp1,$y).',"n2p7":'.$generator->getBaseIDMAP($xm7,$y).',"n3":'.$generator->getBaseIDMAP($x,$ym1).',"n3p7":'.$generator->getBaseIDMAP($x,$ym7).',"n4":'.$generator->getBaseIDMAP($xm1,$y).',"n4p7":'.$generator->getBaseIDMAP($xp7,$y); ?>};
		...
		if($session->plus){
			echo '<a id="map_makelarge" href="#" onclick="PopupMap('.$bigmid.');" ><img class="ml" src="gpack/travian_default/img/x.gif" alt="large map" title="Large Map"/></a>';
		}?>
		<img id="map_navibox" src="gpack/travian_default/img/x.gif" usemap="#map_navibox"/>
		<map name="map_navibox">
			<area id="ma_n1p7" href="karte.php?z=<?php echo $generator->getBaseIDMAP($x,$yp7) ?>" coords="51,15,73,3,95,15,73,27" shape="poly" title="<?php echo NORTH;?>"/>
			<area id="ma_n2p7" href="karte.php?z=<?php echo $generator->getBaseIDMAP($xm7,$y) ?>" coords="51,41,73,29,95,41,73,53" shape="poly" title="<?php echo EAST;?>"/>
			<area id="ma_n3p7" href="karte.php?z=<?php echo $generator->getBaseIDMAP($x,$ym7) ?>" coords="4,41,26,29,48,41,26,53" shape="poly" title="<?php echo SOUTH;?>"/>
			<area id="ma_n4p7" href="karte.php?z=<?php echo $generator->getBaseIDMAP($xp7,$y) ?>" coords="4,15,26,3,48,15,26,27" shape="poly" title="<?php echo WEST;?>"/>
		</map>
		<div id="map_coords">
			<form name="map_coords" method="post" action="karte.php">
				...
				<input type="image" id="btn_ok" class="dynamic_img" value="ok" name="s1" src="gpack/travian_default/img/x.gif" alt="OK" /><br /><br />
