...
foreach($forumcat as $arr){
	...
	if(Alliance::canAct($checkArray)){
		echo '<a class="up_arr" href="allianz.php?s=2&fid='.$arr['id'].'&res=1&admin=pos" title="To top">
			<img src="gpack/travian_default/img/x.gif" alt="To top" /></a><a class="edit" href="allianz.php?s=2&idf='.$arr['id'].'&admin=editforum" title="edit">
			<img src="gpack/travian_default/img/x.gif" alt="edit" /></a><br /><a class="down_arr" href="allianz.php?s=2&fid='.$arr['id'].'&res=0&admin=pos" title="To bottom">
			<img src="gpack/travian_default/img/x.gif" alt="To bottom" /></a><a class="fdel" href="allianz.php?s=2&idf='.$arr['id'].'&admin=delforum" onClick="return confirm(\'confirm delete?\');" title="delete">
			<img src="gpack/travian_default/img/x.gif" alt="delete" /></a>';
	}
	else echo '<img class="folder" src="gpack/travian_default/img/x.gif" title="Thread without new posts" alt="Thread without new posts">';
	...
	if(!empty($owner)){
		echo '<a href="spieler.php?uid='.$owner['id'].'">'.$owner['username'].'</a> <img class="latest_reply" src="gpack/travian_default/img/x.gif" alt="Show last post" title="Show last post" />';
	}
	echo '</td>
		</tr>';
}
...
if(isset($opt['opt5']) && $opt['opt5'] == 1 || $session->access == ADMIN){
	echo '<a href="allianz.php?s=2&admin=newforum"><img id="fbtn_newforum" class="dynamic_img" src="gpack/travian_default/img/x.gif" alt="New forum" /></a>';
	echo '<a href="allianz.php?s='.$ids.((isset($_GET['admin']) && !empty($_GET['admin']) && $_GET['admin'] == "switch_admin") ? "" : "&admin=switch_admin").'" title="Toggle Admin mode"><img class="switch_admin dynamic_img" src="gpack/travian_default/img/x.gif" alt="Toggle Admin mode" /></a>';
}
?>
</p>