...
?>
    <form method="post" action="allianz.php">
        ...
        <div id="box">
            <p><input type="image" value="ok" name="s1" id="btn_ok" class="dynamic_img" src="gpack/travian_default/img/x.gif" alt="OK"></p>
    ...
    <table cellpadding="1" cellspacing="1" id="own" class="dipl">
        ...
        if(count($database->diplomacyOwnOffers($alliance))){
            foreach($database->diplomacyOwnOffers($alliance) as $row){
                echo '<tr><td width="18"><form method="post" action="allianz.php"><input type="hidden" name="o" value="101"><input type="hidden" name="id" value="'.$row['id'].'"><input type="image" class="cancel" src="gpack/travian_default/img/x.gif" title="Cancel" /></form></td><td><a href="allianz.php?aid='.$row['alli2'].'"><center>'.$database->getAllianceName($row['alli2']).'</a></center></td><td width="80"><center>'.(["Conf", "Nap", "War"])[$row['type']-1].'</center></td></tr>';
            }   
        }
    ...
    <table cellpadding="1" cellspacing="1" id="foreign" class="dipl">
        ...
        if(($dInvites = $database->diplomacyInviteCheck($alliance)) && count($dInvites)){
            foreach($dInvites as $row){
                echo '<tr><td width="18"><form method="post" action="allianz.php"><input type="hidden" name="o" value="102"><input type="hidden" name="id" value="'.$row['id'].'"><input type="image" class="cancel" src="gpack/travian_default/img/x.gif" title="Cancel" /></td></form><form method="post" action="allianz.php"><td width="18"><input type="hidden" name="o" value="103"><input type="hidden" name="id" value="'.$row['id'].'"><input type="image" class="accept" src="gpack/travian_default/img/x.gif" title="Accept" /></td></form><td><a href="allianz.php?aid='.$row['alli1'].'"><center>'.$database->getAllianceName($row['alli1']).'</a></center></td><td width="80"><center>'.(["Conf", "Nap", "War"])[$row['type']-1].'</center></td></tr>';
            }   
        } 
        else echo '<tr><td colspan="3" class="none">none</td></tr>';
        ?>
