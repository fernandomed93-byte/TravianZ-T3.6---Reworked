...
</font><br><font color="blue">with all resource,  mainbuilding, rallypoint, warehouse, granary, wall, marketplace, residence, troop (for level-up hero) and one cranny.
</b>
</font>
<p>
<b>Base Name</b> should be between 4 and 20 characters long
    ...
    $initialNameSuggestions = [
        "CityLights", "CottonCandyCloud", "Delightfully", "Dreamer", 
		"Eagle", "EpicQuest", "Farm", "FindMe", "FlyingPancake", "FlySquirrel", 
		"HotSpot", "Intrude", "Logical", "NativeStrong", "PixelArt", 
		"Shakily", "Spectrum", "TechNerd", "TomaHawk", "ZenMaster",
        "SlimeBall", "CupCake", "Satisfying", "Stronghold", "CameraMan",
        "LandScape", "Silhouette", "Odyssey", "Waves", "Light",
        "Road", "Mystique", "Voyager", "Journey", "Andante",
        "Largo", "Outrider", "Canyon", "Boulder", "Ridge",
        "Cavallaro", "Vanguard", "Striker", "Celestial", "Whisper",
        "Realm", "Wanderer", "MysticVortex", "AstralNomad", "LuminousSaga",
        "ArcaneDreamer", "MysticPulse", "StarlitSaga", "FabledAdventurer", "LuminaryKnight",
        "CosmicDrifter", "PhantomSeeker", "RadiantSpecter", "AetherNoir", "EnchantedEcho",
        "MidnightRogue", "FantasyForge", "VelvetMyst", "Thunder", "Alegretto",
        "WispWalker", "SurrealBard", "Manufacturer", "GildedSerpent", "AstralThorn"
    ];

    $filteredNameSuggestions = [];
	foreach ($initialNameSuggestions as $suggestion) {
        $escapedSuggestion = mysqli_real_escape_string($database->dblink, $suggestion);
        $query = "SELECT COUNT(*) as count FROM `" . TB_PREFIX . "users` WHERE `username` LIKE '" . $escapedSuggestion . "%'";
        $result = mysqli_query($database->dblink, $query);
        if ($result) {
            $row = mysqli_fetch_assoc($result);
            if (isset($row['count']) && $row['count'] == 0) {
                $filteredNameSuggestions[] = $suggestion;
            }
        } else {
            $filteredNameSuggestions[] = $suggestion;
            echo "<p style='color:orange;'>Aviso: A consulta para filtrar o nome '{$suggestion}' falhou: " . mysqli_error($database->dblink) . "</p>";
        }
    }

    shuffle($filteredNameSuggestions);


    $baseNameFontColor = "Black";
    $amountFontColor = "Black";
    $baseName = '';
    if (count($filteredNameSuggestions) > 0) $baseName = $filteredNameSuggestions[0];
    ...
    if(isset($_GET['e'])) 
        {
            ...
            switch ($_GET['e'])
            {
				...
				case 'TRNS':
                    $amountFontColor = "Red";
                    echo '<br /><br /><font color="Red"><b>Error: SQL Transaction Error</font></b>';
                    break;
                default:
...
<br><br>
Beginners Protection &nbsp;&nbsp;<input type ="checkbox" name="users_protection" id="users_protection" >
<br><br>
Tribe:<br>
