...
@ini_set('session.cookie_path', '/t3/');
@ini_set('session.name', 'PHPSESSID_T3');
...
try {
	...
	$sql = mysqli_query($connection, "SELECT enableWWstatistics FROM ".TB_PREFIX."config");
	$result = mysqli_fetch_array($sql);
	if($result['enableWWstatistics']==1){ $ww = true; }else{ $ww = false; }
} catch (Exception $e) {
    $ww = false;
}
