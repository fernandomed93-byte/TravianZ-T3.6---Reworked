...
function getStartUnitForTribe($tribe) {
    switch ($tribe) {
        case 1: return 1;  // Romanos
        case 2: return 11; // Teutões
        case 3: return 21; // Gauleses
        case 4: return 31; // Natureza
        case 5: return 41; // Natares
		case 6: return 51; //Huns
		case 7: return 61; //Egyptians
		case 8: return 71; //Spartans
		case 9: return 81; //Vikings
        default: return 1;
    }
}
...
if (!empty($_GET['target'])) {
    ...
    if ($_GET['target'] == 4 || $_GET['target'] == 5) {
	...
	$attacker_tribe = $session->tribe;
	$current_village_id = $village->wid;
	
	// Pega tropas paradas na aldeia
	$units_stationed = $database->getUnit($current_village_id);
	// Pega tropas em movimento
	$units_moving = $database->getVillageMovement($current_village_id);
	$abData = $database->getABTech($current_village_id);
	
	$start_unit = getStartUnitForTribe($attacker_tribe);
	for ($i = 0; $i < 8; $i++) {
		if ($attacker_tribe == 3 && $i == 2) continue;
		if ($attacker_tribe != 3 && $i == 3) continue;

		$unit_id = $start_unit + $i;
		$unit_general_id = $i + 1;
		$unit_key = 'u' . $unit_id;
		
		// Soma as tropas paradas com as tropas em movimento
		$total_units = ($units_stationed[$unit_key] ?? 0) + ($units_moving[$unit_key] ?? 0);
		if ($total_units == 0) {
			$form->setValue('a1_' . $unit_general_id, "");
			$form->setValue('f1_' . $unit_general_id, "");
		}else{
			$form->setValue('a1_' . $unit_general_id, $total_units);
			$form->setValue('f1_' . $unit_general_id, $abData['b' . ($i+1)]);
		}

		$hero_info = $units->Hero($session->uid);
		if ($hero_info){
			$form->setValue('h_off', $hero_info['atk']);
			$form->setValue('h_off_bonus', ($hero_info['ob']-1)*100);
		}
...
<tbody><tr>
	<td>
		...
		<label><input class="radio" type="radio" name="a1_v" value="6" <?php if($tribe == 6) { echo "checked"; } ?>/> Huns</label><br/>
		<label><input class="radio" type="radio" name="a1_v" value="7" <?php if($tribe == 7) { echo "checked"; } ?>/> Egyptians</label><br/>
		<label><input class="radio" type="radio" name="a1_v" value="8" <?php if($tribe == 8) { echo "checked"; } ?>/> Spartans</label><br/>
		<label><input class="radio" type="radio" name="a1_v" value="9" <?php if($tribe == 9) { echo "checked"; } ?>/> Vikings</label><br/>
	...
	<td>
		...
		<label><input class="check" type="checkbox" name="a2_v6" value="1" <?php if(in_array(6,$target)) { echo "checked"; } ?>/> Huns</label><br/>
		<label><input class="check" type="checkbox" name="a2_v7" value="1" <?php if(in_array(7,$target)) { echo "checked"; } ?>/> Egyptians</label><br/>
		<label><input class="check" type="checkbox" name="a2_v8" value="1" <?php if(in_array(8,$target)) { echo "checked"; } ?>/> Spartans</label><br/>
		<label><input class="check" type="checkbox" name="a2_v9" value="1" <?php if(in_array(9,$target)) { echo "checked"; } ?>/> Vikings</label><br/>
